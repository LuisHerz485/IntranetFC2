        ...
        if (value.estado == 0) {
          $('#mostrarCobranza')
            ...
            .row.add([
              ...
              "<button class='btn btn-primary btn-xs btnActivarCo' iddetallecobranza='" +
                value.iddetallecobranza +
                "' idcobranza='" +
                value.idcobranza +
                "' indice=" +
                index +
                " estado='0' onclick=\"limpiarPreConstancia()\" monto='" +
                value.monto +
                "'>Pendiente</button>",
            ])
            .draw(false);
          ...
          $('#mostrarCobranza')
            ...
            .row.add([
              ...
              value.monto,
              value.fechavencimiento,
              "<button class='btn btn-success btn-xs btnActivarCo' iddetallecobranza='" +
                value.iddetallecobranza +
                "' idcobranza='" +
                value.idcobranza +
                "' indice=" +
          ...
          $('#mostrarCobranza')
            ...
            .row.add([
              ...
              value.monto,
              value.fechavencimiento,
              "<button class='btn btn-warning btn-xs btnActivarCo' iddetallecobranza='" +
                value.iddetallecobranza +
                "' idcobranza='" +
                value.idcobranza +
                "' indice=" +
          ...
          $('#mostrarCobranza')
            ...
            .row.add([
              ...
              value.monto,
              value.fechavencimiento,
              "<button class='btn btn-danger btn-xs btnActivarCo' iddetallecobranza='" +
                value.iddetallecobranza +
                "' idcobranza='" +
                value.idcobranza +
                "' indice=" +
                index +
                " estado='3' onclick=\"limpiarPreConstancia()\" monto='" +
                value.monto +
                "'>Vencido</button>",
            ])
            .draw(false);
...
$(document).on('click', '.btnActivarCo', function () {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        ...
        success: function (respuesta) {
          totalCuota = 0;
          $.each(respuesta, function (index, value) {
            $('#mostrarSubPagos > tbody').append(
              '<tr><td>' +
                value.fecha_pago +
                '</td><td>' +
                value.monto_const +
                '</td><td><form action="ajax/generarPDF.php" method="POST" target="_blank"> <input type="hidden" name=idcobranza value="' +
                value.idcobranza +
                '"><button class="btn btn-warning btn-s"> <i class="far fa-file-alt"></i></button> <button class="btn btn-info btn-s btnMostrarHisCob" idconstancia=' +
                value.idconstancia +
                " value='" +
                index +
                '\' data-dismiss="modal" data-toggle="modal" href="#modalDetHisCob"><i class="far fa-eye"></i></button></form></td></tr>'
            );
          ...
          $('#deuda').val(
            (
              Number.parseFloat($(btnSeleccion).attr('monto')) - totalCuota
            ).toFixed(2)
          );
          if (Number.parseFloat($('#deuda').val()) == 0.0) {
...
$('.btnPreConstancia').click(function () {
  ...
  if (
    Number.parseFloat(monto_const) <= Number.parseFloat($('#deuda').val()) &&
    Number.parseFloat(monto_const) > 0.0
  ) {
    if (fecha_pago) {
      if (tipo_pago) {
        $.ajax({
        ...
        if (monto == Number.parseFloat($(btnSeleccion).attr('monto'))) {
          ...
          monto < Number.parseFloat($(btnSeleccion).attr('monto')) &&
          monto > 1
        ) {
  ...
  } else if (Number.parseFloat($('#deuda').val()) == 0.0) {
    Swal.fire('Cliente no tiene monto pendiente!', '', 'info');
  } else {
          ...
          if (result.isConfirmed) {
            ...
            $.ajax({
              ...
              success: function (respuesta) {
                ...
                $.each(respuesta, function (index, value) {
                  $('#mostrarSubPagos > tbody').append(
                    '<tr><td>' +
                      ...
                      " value='" +
                      index +
                      '\' data-dismiss="modal" data-toggle="modal" href="#modalDetHisCob"><i class="far fa-eye"></i></button></from></td></tr>'
                  );
                  totalCuota += Number.parseFloat(value.monto_const);
                });
                $('.btnMostrarHisCob').click(function () {
                ...
                $('#deuda').val(
                  (
                    Number.parseFloat($(btnSeleccion).attr('monto')) -
                    totalCuota
                  ).toFixed(2)
                );
                if (Number.parseFloat($('#deuda').val()) == 0.0) {
